...
open class BaseLicenseReportTask : DefaultTask() {
  ...
  init {
    ...
    outputPath = "${project.buildDir}/reports/licenses/$name".replace('/', File.separatorChar)
    csvFile = File("$outputPath$CSV_EXT")
    htmlFile = File("$outputPath$HTML_EXT")
    jsonFile = File("$outputPath$JSON_EXT")
