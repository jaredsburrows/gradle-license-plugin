...
public final class LicensePlugin extends LicensePluginKt {
  /**
  ...
  @Override protected void configureJavaProject(Project project) {
    ...
    LicenseReportExtension configuration =
      project.getExtensions().create("licenseReport", LicenseReportExtension.class);

    // Create tasks
    LicenseReportTask task = project.getTasks().create(taskName, LicenseReportTask.class);
    task.setDescription("Outputs licenses report.");
    task.setGroup("Reporting");
    task.setHtmlFile(project.file(path + LicenseReportTask.getHTML_EXT()));
    task.setJsonFile(project.file(path + LicenseReportTask.getJSON_EXT()));
  ...
  @Override protected void configureAndroidProject(Project project) {
    ...
    DomainObjectCollection<? extends com.android.build.gradle.api.BaseVariant> variants =
      getAndroidVariant(project);
    LicenseReportExtension configuration =
      project.getExtensions().create("licenseReport", LicenseReportExtension.class);

    // Configure tasks for all variants
    variants.all(variant -> {
      // TODO make sure this is capitalized
      ...
      task.setAssetDirs(project.getExtensions()
        .getByType(com.android.build.gradle.BaseExtension.class)
        .getSourceSets()
        .getByName("main")
        .getAssets()
        .getSrcDirs());
      task.setBuildType(variant.getBuildType().getName());
      task.setVariant(variant.getName());
      task.setProductFlavors(variant.getProductFlavors());
      // Make sure update on each run
      task.getOutputs().upToDateWhen(element -> false);
    });
  }
